...
数据说明 (见 fixtures/zengshan_230_cases.py):
  ...
  当前在"数据已核验"的 19 例上命中 13 例 (68.4%), 为真实可信的准确度样本。
"""
...
#
# 在"数据已核验"的 19 例上, 当前命中 13 例 = 68.4% (真实可信准确度)。
BASELINE_HIT_IDS = {
    # —— 数据已核验且命中 (13 例, 真实准确度的可信样本) ——
    "例1", "例3", "例6", "例7", "例8", "例11", "例12",
...
KNOWN_MISMATCH = {
    ...
    "例5": "(B) 三合局克用神 + 化刑定凶未识别 (引擎判吉)",
    "例9": "(B) 月令时效卦临月建逢克不凶未实现 (引擎判凶, 需问事时效上下文)",
    "例10": "(B) 三合局生世优先于单爻分析未落定为吉 (引擎判凶)",
