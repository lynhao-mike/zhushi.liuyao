...
数据说明 (见 fixtures/zengshan_230_cases.py):
  ...
  - 其中 19 例的 ben_gua/bian_gua 取自原文, yao_types 已据"动爻位 = 本卦与
    变卦阴阳相异处"逐例校正核验 (此前多例 yao_types 与卦名不符, 已修正);
  - 另 11 例 ben_gua/bian_gua 仍为 FIXME, yao_types 为占位值, 待按原书补全;
  - 个别案例数据有误 (如 例108 在亥日却以亥为旬空, 自相矛盾), 计入已知未对照集。

  当前在"数据已核验"的 19 例上命中 14 例 (73.7%), 为真实可信的准确度样本。
"""
...
# 命中基线与已知未对照集 (回归红线)                                              #
# ============================================================================ #
# 说明: fixtures 中 19 例的 ben_gua/bian_gua 取自《增删卜易》原文, 其 yao_types
# 已据"动爻位 = 本卦与变卦阴阳相异处"逐例校正核验(详见 fixtures 文件内注释);
# 另 11 例 ben_gua/bian_gua 仍为 FIXME, yao_types 为占位值, 待按原书补全。
#
# 以原文真实日月干支复盘后, 当前引擎吉凶判定与原例一致的案例集合(回归红线)。
# 这些案例为硬性守卫: 任何改动若令其判定退化, 测试立即失败。
#
# 在"数据已核验"的 19 例上, 当前命中 14 例 = 73.7% (真实可信准确度)。
BASELINE_HIT_IDS = {
    # —— 数据已核验且命中 (14 例, 真实准确度的可信样本) ——
    "例1", "例3", "例6", "例7", "例8", "例10", "例11", "例12",
    "例14", "例15", "例17", "例20", "例22", "例23",
    # —— 占位数据(FIXME)恰好命中 (5 例, 待原书核实, 数据补全后可能变动) ——
    "例38", "例44", "例60", "例61", "例218",
}

# 当前尚未达成对照的案例及原因。修复后应将其移入 BASELINE_HIT_IDS。
# 原因分两类:
#   (A) fixture 数据待核实 (ben_gua/bian_gua 为 FIXME, yao_types 为占位值);
#   (B) 数据已核验, 引擎卦理实现存在缺口 (这些是真正可着手修复的引擎问题)。
KNOWN_MISMATCH = {
    # —— (B) 数据已核验的真实引擎缺口 ——
    "例2": "(B) 变爻用神生世吉局未识别 (引擎判凶, 期望吉)",
    "例4": "(B) 动化回头生胜日月克(动兆为主)未落定为吉 (引擎判凶)",
    "例5": "(B) 代占用神选择: 占父之事用神应取父母而非按问类取官鬼 "
           "(用神取父母时三合木局克父母可正确判凶, 现按官鬼判吉)",
    "例9": "(B) 月令时效卦临月建逢克不凶未实现 (引擎判凶, 需问事时效上下文)",
    "例18": "(B) 金刚型特殊日月组合下回头克不伤仍有用未实现 (引擎判平)",
    # —— (A) fixture 数据待核实 (ben_gua/bian_gua 为 FIXME) ——
    "例41": "(A) ben_gua/bian_gua 为 FIXME, yao_types 为占位值, 待原书核实",
    "例54": "(A) ben_gua/bian_gua 为 FIXME, yao_types 为占位值, 待原书核实",
    "例101": "(A) ben_gua/bian_gua 为 FIXME, yao_types 为占位值, 待原书核实",
    "例144": "(A) ben_gua/bian_gua 为 FIXME, yao_types 为占位值, 待原书核实",
    "例205": "(A) ben_gua/bian_gua 为 FIXME, yao_types 为占位值, 待原书核实",
    "例108": "(A) fixture 数据错误: 亥日不可能以亥为旬空, 且卦名为 FIXME, 待原书核实",
}
